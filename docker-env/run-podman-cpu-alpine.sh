...
# ==============================================================================
# Detect container runtime, build and run CPU vanity key image (Alpine)
# ==============================================================================
# Usage: ./run-podman-cpu-alpine.sh [pattern] [--patterns-file <file>] [-o <path>] [args...]
#   - No GPU required. Base: alpine:3.20 + python3 + py3-cryptography
# ==============================================================================
...
main() {
    ...
    if [[ $# -eq 0 ]]; then
        ...
        echo "  CPU-only vanity key generator (Alpine). No GPU required."
    ...
    local dockerfile="${SCRIPT_DIR}/Dockerfile.cpu-alpine"
    local image_tag="aminuxer-ssh-ed25519-cpu-vanity-alpine:latest"
    ...
    if $CONTAINER_TOOL container inspect ssh-25519-vanity-cpu-alpine &>/dev/null; then
        echo "Removing stale container ssh-25519-vanity-cpu-alpine ..."
        $CONTAINER_TOOL rm -f ssh-25519-vanity-cpu-alpine 2>/dev/null || true
    fi
